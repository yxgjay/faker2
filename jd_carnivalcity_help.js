...
!(async () => {
  ...
  $.updatePkActivityIdRes = await getAuthorShareCode('https://127.0.0.1/jd_cityShareCodes.json')
  if (!$.updatePkActivityIdRes) {
    $.http.get({url: 'https://127.0.0.1/jd_cityShareCodes.json'}).then((resp) => {}).catch((e) => $.log('刷新CDN异常', e));
    await $.wait(1000)
    $.updatePkActivityIdRes = await getAuthorShareCode('https://127.0.0.1/jd_cityShareCodes.json')
  }
