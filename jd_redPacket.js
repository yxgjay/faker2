...
!(async () => {
  ...
  let res = await getAuthorShareCode('https://127.0.0.1/jd_red.json')
  if (!res) {
    $.http.get({url: 'https://127.0.0.1/jd_red.json'}).then((resp) => {}).catch((e) => $.log('刷新CDN异常', e));
    await $.wait(1000)
    res = await getAuthorShareCode('https://127.0.0.1/jd_red.json') 
  }  
