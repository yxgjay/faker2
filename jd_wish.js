...
!(async () => {
  ...
  let res = await getAuthorShareCode('https://127.0.0.1/wish.json')
  if (!res) {
    $.http.get({url: 'https://127.0.0.1/wish.json'}).then((resp) => {}).catch((e) => console.log('刷新CDN异常', e));
    await $.wait(1000)
    res = await getAuthorShareCode('https://127.0.0.1/wish.json')
  }
